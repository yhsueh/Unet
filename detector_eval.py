...
def main():
	...
	with Image.open('./dataset/test_img/crop_19.png').convert('L') as im:
        ...
        torch_im = torch.unsqueeze(torch_im, dim=0) #[sample, channel, height, width]
        ...
        probs = F.softmax(output, dim=1).squeeze(dim=0)[1]
        mask = probs.numpy()    
